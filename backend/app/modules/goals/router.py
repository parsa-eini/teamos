...
@router.get(
    ...
    summary="List goals",
    description=(
        "List goals in the current organization. Members see goals they own; managers see "
        "organization-wide goals, goals on their teams, and goals they created or own."
    ),
...
@router.post(
    ...
    description=(
        ...
        "A goal may target several teams and have several owners. Linking tasks makes progress "
        "derived from those tasks, so progress must not be supplied alongside task_ids."
    ),
    responses={
        **_AUTH_RESPONSES,
        404: {"model": ErrorResponse, "description": "Team, user, or task not found"},
...
@router.patch(
    ...
    summary="Update goal",
    description=(
        "Update a goal. Owners may update any goal; managers may update goals they can view. "
        "Progress may be set by hand (0-100) only while the goal has no linked tasks; once "
        "tasks are linked, progress is the share of them that is done."
    ),
    responses={
        **_AUTH_RESPONSES,

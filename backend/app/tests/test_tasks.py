...
from app.modules.tasks.schemas import MAX_TASK_ASSIGNEES
...
def test_owner_can_crud_task(client: TestClient) -> None:
    ...
    assert task["assignee_ids"] == [str(owner["id"])]
...
def test_task_can_carry_several_assignees(client: TestClient, app: FastAPI) -> None:
    owner = _register(client, email="owner@example.com", organization_name="Acme")
    member = _register(client, email="member@example.com", organization_name="Other")
    _move_user_to_owner_org(app, "owner@example.com", "member@example.com", OrganizationRole.MEMBER)
    token = _login(client, "owner@example.com")
    ...
    assert task["assignee_ids"] == sorted([owner_id, member_id])

    # The filter matches any task that includes the person, not just a sole assignee.
    for user_id in (owner_id, member_id):
        listed = client.get(f"/api/v1/tasks?assignee_id={user_id}", headers=_auth(token))
        assert [item["id"] for item in listed.json()["data"]] == [task["id"]]

    narrowed = client.patch(
        f"/api/v1/tasks/{task['id']}",
        headers=_auth(token),
        json={"assignee_ids": [member_id]},
    )
    ...
    assert cleared.json()["data"]["assignee_ids"] == []


def test_assignee_ids_come_back_in_a_stable_order(client: TestClient, app: FastAPI) -> None:
    owner = _register(client, email="owner@example.com", organization_name="Acme")
    member = _register(client, email="member@example.com", organization_name="Other")
    _move_user_to_owner_org(app, "owner@example.com", "member@example.com", OrganizationRole.MEMBER)
    token = _login(client, "owner@example.com")
    ...
    forward = _create_task(client, token, project_id, "Forward", assignee_ids=ids)
    reverse = _create_task(client, token, project_id, "Reverse", assignee_ids=list(reversed(ids)))

    assert forward["assignee_ids"] == reverse["assignee_ids"]
    assert forward["assignee_ids"] == sorted(ids)
    refetched = client.get(f"/api/v1/tasks/{forward['id']}", headers=_auth(token))
    assert refetched.json()["data"]["assignee_ids"] == forward["assignee_ids"]


def test_create_task_deduplicates_assignees(client: TestClient) -> None:
    owner = _register(client, email="owner@example.com", organization_name="Acme")
    token = _login(client, "owner@example.com")
    project_id = _create_project(client, token, "Launch")
    owner_id = str(owner["id"])
    ...
    assert task["assignee_ids"] == [owner_id]


def test_create_task_rejects_too_many_assignees(client: TestClient) -> None:
    _register(client, email="owner@example.com", organization_name="Acme")
    token = _login(client, "owner@example.com")
    project_id = _create_project(client, token, "Launch")
    ...
    response = client.post(
        ...
        json={
            "project_id": project_id,
            "title": "Crowded",
            "assignee_ids": [str(uuid4()) for _ in range(MAX_TASK_ASSIGNEES + 1)],
        },
    )
...
def test_cannot_assign_any_user_from_another_organization(client: TestClient) -> None:
    owner = _register(client, email="a@example.com", organization_name="Acme")
    outsider = _register(client, email="b@example.com", organization_name="Globex")
    token_a = _login(client, "a@example.com")
    project_id = _create_project(client, token_a, "Launch")
    ...
    response = client.post(
        ...
        json={
            "project_id": project_id,
            "title": "Mixed",
            "assignee_ids": [str(owner["id"]), str(outsider["id"])],
        },
    )
    assert response.status_code == 404
    assert response.json()["error"]["code"] == "RESOURCE_NOT_FOUND"
    assert client.get("/api/v1/tasks", headers=_auth(token_a)).json()["meta"]["total"] == 0


def test_every_assignee_can_see_and_update_a_shared_task(client: TestClient, app: FastAPI) -> None:
    owner = _register(client, email="owner@example.com", organization_name="Acme")
    member = _register(client, email="member@example.com", organization_name="Other")
    _move_user_to_owner_org(app, "owner@example.com", "member@example.com", OrganizationRole.MEMBER)
    owner_token = _login(client, "owner@example.com")
    ...
    shared = _create_task(
        ...
        project_id,
        "Shared",
        assignee_ids=[str(owner["id"]), str(member["id"])],
    )
    ...
    updated = client.patch(
        f"/api/v1/tasks/{shared['id']}",
        headers=_auth(member_token),
        json={"status": "IN_PROGRESS"},
    )
    assert updated.status_code == 200

    # A co-assignee still may not change who else is on the task.
    assert (
        client.patch(
            f"/api/v1/tasks/{shared['id']}",
            headers=_auth(member_token),
            json={"assignee_ids": [str(member["id"])]},
        ).status_code
        == 403
    )

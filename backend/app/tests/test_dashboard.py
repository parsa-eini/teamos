...
def test_dashboard_aggregates_scoped_to_the_current_organization(
    ...
    assert data["recent_meetings"][0]["id"] == meeting_id
    assert data["recent_meetings"][0]["status"] == "SUBMITTED"
    assert data["recent_meetings"][0]["type"] == "CHECK_IN"
    assert data["recent_meetings"][0]["scheduled_on"] == "2026-08-07"
    assert data["recent_meetings"][0]["member_id"] == str(member["id"])

    messages = [item["message"] for item in data["recent_activity"]]
    assert 'Alex User completed "API redesign"' in messages
    assert "Alex User submitted a check-in" in messages
    assert "Alex User created a new project" in messages
    assert all("Foreign" not in message for message in messages)


def test_dashboard_activity_names_the_meeting_type(client: TestClient, app: FastAPI) -> None:
    _register(client, email="owner@example.com", organization_name="Acme")
    member = _register(client, email="member@example.com", organization_name="Other")
    _move_user_to_owner_org(app, "owner@example.com", "member@example.com", OrganizationRole.MEMBER)
    owner_token = _login(client, "owner@example.com")
    ...
    client.patch(
        f"/api/v1/meetings/{review.json()['data']['id']}",
        headers=_auth(member_token),
        json={"status": "SUBMITTED"},
    )

    data = client.get("/api/v1/dashboard", headers=_auth(owner_token)).json()["data"]
    assert data["recent_meetings"][0]["type"] == "PERFORMANCE_REVIEW"
    messages = [item["message"] for item in data["recent_activity"]]
    assert "Alex User submitted a performance review" in messages

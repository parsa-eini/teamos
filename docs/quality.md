...
| meetings | `app/tests/test_meetings.py` |
...
Critical panel workflows (login, dashboard, teams, projects, tasks, goals, meetings, feedback,
organization members, settings, and route guards) are covered with Vitest and Testing Library. The public website
...
| Indexes | Besides primary keys, unique constraints, and foreign-key indexes, composite indexes cover dashboard and filter paths: tasks `(organization_id, status)` and `(organization_id, status, due_date)`, projects `(organization_id, status)`, goals and meetings `(organization_id, updated_at)`, meetings `(organization_id, type, scheduled_on)`, feedback `(organization_id, subject_id, created_at)`, notifications `(user_id, created_at)`. |
